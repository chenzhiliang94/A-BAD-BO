...
def find_prompts_simple(eval_template,
                 demos_template,
                 prompt_gen_data,
                 eval_data,
                 conf,
...
def ape_to_produce_prompt_autoAI(dataset,
               eval_template='Instruction: [PROMPT]\nInput: [INPUT]\nOutput: [OUTPUT]',
               prompt_gen_template=None,
               demos_template='Input: [INPUT]\nOutput: [OUTPUT]',
               eval_model='gpt-3.5-turbo',
    ...
    prompt_gen_template = get_simple_prompt_gen_template(
        prompt_gen_template, prompt_gen_mode)
    conf = config.simple_config(
        eval_model, prompt_gen_model, prompt_gen_mode, num_prompts, 1, prompt_gen_batch_size, 1)
    return find_prompts_simple(eval_template, demos_template, dataset, dataset, conf, prompt_gen_template=prompt_gen_template)


def evaluate_prompts_autoAI(prompts, eval_data, demo_data,
               eval_template='Instruction: [PROMPT]\nInput: [INPUT]\nOutput: [OUTPUT]',
               demos_template='Input: [INPUT]\nOutput: [OUTPUT]',
               eval_model='gpt-3.5-turbo',
               prompt_gen_model='gpt-3.5-turbo',
    ...
    conf = config.simple_config(    
        eval_model, prompt_gen_model, prompt_gen_mode, num_prompts, 1, prompt_gen_batch_size, 1)
    result = evaluate_prompts(prompts, eval_template, eval_data, demos_template, demo_data, conf,
                     base_conf='configs/default.yaml')
    return result
...
def evaluate_prompts(prompts, eval_template, eval_data, demos_template, few_shot_data, conf,
    ...
    print('Evaluating prompts...')
    res = evaluate.evalute_prompts(
        prompts, eval_template, eval_data, demos_template, few_shot_data, "exec_accuracy_evaluator",
        conf['evaluation'])
